        ...
        (function($) {
            $(function() {
                $('#namaToko').select2({
                    theme: 'bootstrap-5',
                    width: '100%',
                    placeholder: $('#namaToko').data('placeholder'),
                    allowClear: true
                });
                ...
                $('#marketplace').on('change', function() {
                    ...
                    $.get("{{ route('pesanan.getToko', ':market') }}".replace(':market', market),
                        function(res) {
                            ...
                            (response.data || []).forEach(function(item, index) {
                                (item.produk_detail || []).forEach(function(produk, i) {
                                    ...
                                    if (produk['Harga'] != null) {
                                        hargaNum = String(produk['Harga']).replace(
                                            /[^\d]/g, '');
                                    }

                                    let skuVal = (
                                        (produk['sku'] != null ? String(produk[
                                            'sku']) : '') ||
                                        (produk['__sku'] != null ? String(produk[
                                            '__sku']) : '') ||
                                        (produk['SKU Induk'] != null ? String(
                                            produk['SKU Induk']) : '') ||
                                        (produk['SKU'] != null ? String(produk[
                                            'SKU']) : '')
                                    ).trim();
